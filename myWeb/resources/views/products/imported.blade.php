...
@section('title', 'Danh sách trái cây nhập khẩu')
...
@section('content')
<h1 class="text-2xl mb-4">Danh sách trái cây nhập khẩu</h1>
  ...
  @foreach($products as $index => $p)
    <div class="bg-white p-4 rounded shadow flex flex-col justify-between product-card">
      <a href="{{ route('product.show', $p->slug) }}">
        <img
          src="{{ asset('storage/'.$p->image) }}"  {{-- Đảm bảo sử dụng đúng đường dẫn ảnh sản phẩm --}}
          alt="{{ $p->name }}"
